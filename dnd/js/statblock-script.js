...
function UpdateStatblock(moveSeparationPoint) {
    ...
    if (mon.isLegendary && (mon.legendaries.length > 0 || mon.legendariesDescription.length > 0))
        AddToTraitList(traitsHTML, mon.legendaries, mon.legendariesDescription == "" ?
            "<h3>Villain Actions</h3><div class='property-block'></div>" :
            ["<h3>Villain Actions</h3><div class='property-block'>", StringFunctions.FormatString(ReplaceTags(StringFunctions.RemoveHtmlTags(mon.legendariesDescription))), "</div></br>"], true);
...
function BuildMarkdown(isV3Markdown) {
    ...
    if (mon.isLegendary) {
        AddMarkdownTraitSection(markdownLines, isV3Markdown, "Villain Actions", mon.legendaries, mon.legendariesDescription, null, LEGENDARY);
...
var GetVariablesFunctions = {
    ...
    LegendaryDescriptionDefault: function () {
        mon.legendariesDescription = "The " + mon.name.toLowerCase() + " has three villain actions. They can take each action once during an encounter after an enemy's turn. They can take these actions in any order but can use only one per round.";
    },
